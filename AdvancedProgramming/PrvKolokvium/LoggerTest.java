...
class LogSystem<T extends ILog> {
    ...
    void printResults() {
        ...
        LogProcessor<T> secondLogProcessor = logs -> logs.stream().filter(log -> log.getMessage().length() <= 100).collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<T> thirdLogProcessor = logs -> logs.stream().sorted(Comparator.comparing(ILog::getTimestamp)).collect(Collectors.toCollection(ArrayList::new));
