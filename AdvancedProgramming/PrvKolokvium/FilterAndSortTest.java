...
class FilterAndSort<T extends Comparable<T>> {
    public static <T extends Comparable<T>> void execute(List<T> items, Predicate<T> predicate) throws EmptyResultException {
    ...
    public static void main(String[] args) throws EmptyResultException {
        ...
        if (testCase == 1) { // students
            ...
            if (studentScenario == 1) {
                ...
                try {
                    FilterAndSort.execute(students, student -> student.labAssistantPoints() >= 8.0 && student.getYear() >= 3);
                ...
                try {
                    FilterAndSort.execute(students, student -> student.grades.size() >= 0.9 * student.totalCourses() && student.average() >= 9.0 && student.labAssistantPoints() >= 7.50);
                } catch (EmptyResultException e) {
                    System.out.println(e.getMessage());
                    ;
                }
